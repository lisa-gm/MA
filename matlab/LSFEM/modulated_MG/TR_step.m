...
function [sigma, u, norm_grad_J, rho, TR_rad] = TR_step(TR_rad, TR_rad_max, eta, max_iter_TR, levels, smoother, max_iter_mg, S, Nx_elem, T, Nt_elem, u0, bdy_cond, bdy_left, bdy_right, sigma, u, update, grad_J, Hess_J, J_ss_lin, J_su_lin, J_us_lin, J_uu_lin, c1, c2, diff_const, iter)
eps = 10^(-9);
%%%%%%%%%%%%%%% SET UP %%%%%%%%%%%%%%
int_space = [0,S];
...
%         norm(u)
        ...
        if(norm_grad_J < 10^(-9))
            save('final_Hess_J_inner.mat', 'Hess_J_inner');
            save('final_grad_J_inner.mat', 'grad_J_inner');
            
            sol = [sigma; u];
            save('final_sol.mat', 'sol');
