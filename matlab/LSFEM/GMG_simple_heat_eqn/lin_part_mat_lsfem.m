...
function [J_ss_lin, J_su_lin, J_us_lin, J_uu_lin] = lin_part_mat_lsfem (S, Nx_elem, T, Nt_elem, c1 ,c2, diff_const, bdy_cond)
...
J_uu_lin(1:Nx_pts,:) = 0;
J_us_lin(1:Nx_pts,:) = 0;
J_uu_lin(1:Nx_pts, 1:Nx_pts) = eye(Nx_pts, Nx_pts);


if(strcmp(bdy_cond, 'Dirichlet'))
    J_uu_lin(1:Nx_pts:end) = 0;
    J_uu_lin(Nx_pts:Nx_pts:end) = 0;

    J_us_lin(1:Nx_pts:end) = 0;
    J_us_lin(Nx_pts:Nx_pts:end) = 0;

    J_uu_lin(1:Nx_pts:end, 1:Nx_pts:end) = eye(Nt_pts, Nt_pts);
    J_uu_lin(Nx_pts:Nx_pts:end, Nx_pts:Nx_pts:end) = eye(Nt_pts, Nt_pts);
end


if(strcmp(bdy_cond, 'Neumann'))
    J_ss_lin(1:Nx_pts:end) = 0;
    J_ss_lin(Nx_pts:Nx_pts:end) = 0;

    J_su_lin(1:Nx_pts:end) = 0;
    J_su_lin(Nx_pts:Nx_pts:end) = 0;

    J_ss_lin(1:Nx_pts:end, 1:Nx_pts:end) = eye(Nt_pts, Nt_pts);
    J_ss_lin(Nx_pts:Nx_pts:end, Nx_pts:Nx_pts:end) = eye(Nt_pts, Nt_pts);
end
