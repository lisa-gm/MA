...
function [J_eval, J_eval_vec] = J_eval_lsfem(hx, Nx_elem, ht, Nt_elem, sigma, u, c1, c2, diff_const)
...
t1_vec = zeros(tot_pts,1);
t2_vec = zeros(tot_pts, 1);
     ...
     [coeff_vec_f,~, ~] = F_eval(coeff_vec_u);
        ...
        for pt_j=1:qd_deg
            ...
            for pt_i=1:qd_deg             
               ...
               %t1_vec(curr_ind(bf_j)) = t1_vec(curr_ind(bf_j)) + 0.5*c1*hxht*(coeff_vec_u(bf_j)*grad_v_t_eval(bf_j) - coeff_vec_sigma(bf_j)*grad_tau_x_eval(bf_j) - coeff_vec_f(bf_j)*bf_eval(bf_j))...
                %   *(coeff_vec_u(bf_i)*grad_v_t_eval(bf_i) - coeff_vec_sigma(bf_i)*grad_tau_x_eval(bf_i) - coeff_vec_f(bf_i)*bf_eval(bf_i))*qd_weights(pt_i)*qd_weights(pt_j);           
               ...
               %t2_vec(curr_ind(bf_j)) = t2_vec(curr_ind(bf_j)) + 0.5*c2*hxht*(coeff_vec_sigma(bf_j)*bf_eval(bf_j)-diff_const*coeff_vec_u(bf_j)*grad_tau_x_eval(bf_j))...
                %       *(coeff_vec_sigma(bf_i)*bf_eval(bf_i)-diff_const*coeff_vec_u(bf_i)*grad_tau_x_eval(bf_i))*qd_weights(pt_i)*qd_weights(pt_j);
...
%J_eval = sum(t1_vec) + sum(t2_vec);
J_eval_vec = t1_vec + t2_vec;
