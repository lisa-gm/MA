...
% MG parameters
max_iter = 9000;                          % number of newton iterations
eps = 10^(-8);                           % tolerance
...
sol_Jac_sub = JacobiSolve_LS_extended_SP_T(Nx_pts, Nt_pts, Hess_J_sym, rhs_sym, sol_init, max_iter, omega, block_size_s, block_size_t);

sigma_Jac_sub = sol_Jac_sub(1:tot_pts);
u_Jac_sub = sol_Jac_sub(tot_pts+1:end);
...
norm_diff_sol = norm(sol_direct - [sigma_GS; u_GS]);
...
u_mat_GS = zeros(Nx_pts, Nt_pts);


for ts=1:Nt_pts
    u_mat_direct(:, ts) = u_direct((ts-1)*Nx_pts+1:ts*Nx_pts);
    u_mat_GS(:, ts) = u_GS((ts-1)*Nx_pts+1:ts*Nx_pts);
end

figure
mesh(x_vec, t_vec, transpose(u_mat_GS));
xlabel('space');
ylabel('time');
zlabel('u');
%zlim([min(u), max(u)]);
