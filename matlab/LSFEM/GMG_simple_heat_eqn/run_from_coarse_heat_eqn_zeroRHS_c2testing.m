...
% MG parameters
max_iter = 800;                          % number of newton iterations
eps = 10^(-9);                           % tolerance
levels = 2; 
...
a = 1;
b = -1;
diff_const = a*10^(b);                          % diffusion constant, define through sigma = diff_const*nabla(u) 


% LSFEM parameters
c1 = 1;                                 % constants in front of J = c1*|| ....-f(u) ||^2 + c2*|| ...||^2
c2 = 10;                                 % need to multiply with them in computation of each integral 
...
[sol_mg_P, it_sol_mg_P, it_res_mg_P] = V_cycle_galerkin_assembly_P(Nx_elem_list, hx_list, Nt_elem_list, ht_list, Hess_J, f, sol_init, bdy_cond, u0, bdy_left, bdy_right, levels, max_iter, smoother, eps);
...
file_name = ['data_',num2str(Nx_elem_list(1)),'by', num2str(Nt_elem_list(1)),'_fine_elem_c2_', num2str(c2),'_d_', num2str(a),'_10_',num2str(b),'.mat'];
save([path, file_name], 'it_sol_mg', 'sol_direct', 'it_res_mg', 'conv_coeff_res', 'Hess_J_sym');
...
% 
fprintf('geometric mean value res conv coeff : %d \n', geomean(conv_coeff_res));
% % ************************ PLOTTING ******************************** %
% 
% % reshape
