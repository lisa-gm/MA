...
function [sigma, u, norm_grad_J, rho, TR_rad, conv_data] = TR_step(TR_rad, TR_rad_max, eta, max_iter_TR, S, Nx_elem, T, Nt_elem, u0, bdy_cond, bdy_left, bdy_right, sigma, u, update, grad_J, Hess_J, J_ss_lin, J_su_lin, J_us_lin, J_uu_lin, c1, c2, diff_const, iter)
eps = 10^(-9);
%%%%%%%%%%%%%%% SET UP %%%%%%%%%%%%%%
int_space = [0,S];
...
% data on convergence containing
% accepted_iter, total_iter, norm J, grad norm J, norm update, rho, TR_rad --> 6
conv_data = zeros(max_iter_TR, 7);
...
for k=1:max_iter_TR
    ...
    if(rho > eta)
        ...
        fprintf('accepted iter: %g, overall iter %g, norm J = %1.15d, norm grad J = %1.15d, norm update = %g, rho = %g, TR_rad =%g\n', counter, k, J_eval, norm_grad_J, norm([update_sigma; update_u]), rho, TR_rad);
        counter = counter + 1;
        
        % write table, only count accepted steps
        conv_data(counter, :) = [counter, k, J_eval, norm_grad_J, norm([update_sigma; update_u]), rho, TR_rad]; 
        
        % break out of TR_step to recompute a new Hessian, gradient, update
        %return;
        
        if(norm_grad_J < 1*10^(-9))
            % save last Hess_J matrix so that it can be used for the
            % linearisation
            file_name = ['Hess_J_', num2str(Nx_elem), 'by', num2str(Nt_elem), 'elem_diff0_001.mat'];
            save(file_name, 'Hess_J', 'grad_J', 'S', 'Nx_elem', 'T', 'Nt_elem','sigma', 'u', 'u0', 'bdy_cond', 'bdy_left', 'bdy_right','diff_const');
