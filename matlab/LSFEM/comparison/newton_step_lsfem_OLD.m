...
function [sigma, u, norm_grad_J, damp_fact] = newton_step_lsfem(S, Nx_elem, T, Nt_elem, u0, bdy_cond, bdy_left, bdy_right, sigma, u, a0, a1, a2, a3, alpha, J_ss_lin, J_su_lin, J_us_lin, J_uu_lin, c1, c2, diff_const, eps, damp_fact, iter)
max_iter = 50;
sol = [sigma; u];
...
% f_eval_fct = @y y.*y;
% df_eval_fct = @y 2.*y;
% d2f_eval_fct = @y 2*ones(size(y));


%d2f_dep_xt = @(coeff_vec,x,t, pt_i, pt_j) 0;
%df_dep_xt = @(coeff_vec,x,t, pt_i, pt_j) 0;
% ARRAY ALLOCATION 
...
%      coeff_vec_df = df_eval(coeff_vec_u);
%      coeff_vec_d2f = d2f_eval(coeff_vec_u);
%      
     ...
     temp_vec_1 = zeros(4,1);
        ...
        for pt_j=1:qd_deg
            ...
            for pt_i=1:qd_deg             
                ...
                d2f_local = dot(coeff_vec_d2f, bf_eval); 
                df_local = dot(coeff_vec_df, bf_eval);
                f_local = dot(coeff_vec_f, bf_eval);                
                ...
                for bf_j=1:4
                    for bf_i=1:4 
                        ...
                        temp_vec_1(bf_j) = temp_vec_1(bf_j) + c1*hxht*grad_tau_x_eval(bf_j)*coeff_vec_f(bf_i)*bf_eval(bf_i)*qd_weights(pt_i)*qd_weights(pt_j);
                        
                        temp_1(bf_i, bf_j) = temp_1(bf_i, bf_j) + c1*hxht*grad_v_t_eval(bf_i)*bf_eval(bf_j)*qd_weights(pt_i)*qd_weights(pt_j);
                        % temp_2(bf_i, bf_j) = temp_2(bf_i, bf_j) + c1*hxht*grad_tau_x_eval(bf_i)*coeff_vec_df(bf_j)*bf_eval(bf_j)*qd_weights(pt_i)*qd_weights(pt_j);
                    ...
                    v_t_f_int_vec(curr_ind(bf_j)) = v_t_f_int_vec(curr_ind(bf_j)) ...
                       ...
                       temp_2(bf_i, bf_i) = temp_2(bf_i, bf_i) + c1*hxht*bf_eval(bf_i)*coeff_vec_d2f(bf_i)*coeff_vec_f(bf_j)*bf_eval(bf_j)*qd_weights(pt_i)*qd_weights(pt_j);
                       ...
                       u_t_d2f_int(curr_ind(bf_i), curr_ind(bf_j)) = u_t_d2f_int(curr_ind(bf_i), curr_ind(bf_j)) ...
                                          - c1*hxht*bf_eval(bf_i)*bf_eval(bf_j)*u_t_local*coeff_vec_d2f(bf_j)*qd_weights(pt_i)*qd_weights(pt_j);
                        
                       % compute <sigma_x, w^t f''(u) v>
                       s_x_d2f_int(curr_ind(bf_i), curr_ind(bf_j)) = s_x_d2f_int(curr_ind(bf_i), curr_ind(bf_j)) ...
                                          + c1*hxht*bf_eval(bf_i)*bf_eval(bf_j)*sigma_x_local*coeff_vec_d2f(bf_j)*qd_weights(pt_i)*qd_weights(pt_j); 
...
save('comp_u.mat', 'components_u');
save('comp_sigma.mat', 'components_sig');
...
fprintf('norm J_sigma: %d, iteration: %d\n ', norm(J_s), iter);
fprintf('norm J_u: %d, iteration: %d\n ', norm(J_u), iter);
% 
% fprintf('norm J_s_lin: %d\n ', norm(J_ss_lin*sigma + J_su_lin*u));
% fprintf('norm J_u_lin: %d\n ', norm(J_us_lin*sigma + J_uu_lin*u));

% fprintf('norm J_sigma tau_x_f_int_vec: %d, iteration: %d\n ', norm(tau_x_f_int_vec), iter);

J_u_us = J_us_lin*sigma;
J_u_uu = J_uu_lin*u;

% fprintf('norm  v_t_f_int_vec: %d, \n ', norm( v_t_f_int_vec));
% fprintf('norm  u_t_df_int_vec: %d, \n ', norm( u_t_df_int_vec));
% fprintf('norm sigma_x_df_int_vec: %d, \n ', norm(sigma_x_df_int_vec));
% fprintf('norm f_df_int_vec: %d, \n ', norm(f_df_int_vec));

% fprintf('norm Jvv_lin: %d\n', norm(J_ss_lin(inner_ind_sigma, inner_ind_sigma)));
% fprintf('norm Jvu_lin: %d\n', norm(J_su_lin(inner_ind_sigma, inner_ind_u)));
% fprintf('norm Juv_lin: %d\n', norm(J_us_lin(inner_ind_u, inner_ind_sigma)));
% fprintf('norm Juu_lin: %d\n', norm(J_uu_lin(inner_ind_u, inner_ind_u))); 
...
components_J_uu = [J_uu_lin, df_df_int, f_d2f_int, u_t_d2f_int, s_x_d2f_int, v_t_df_int_1, v_t_df_int_2];

save('comp_J_uu.mat', 'components_J_uu');
...
% fprintf('norm Jvu: %d\n', norm(J_su(inner_ind_sigma, inner_ind_u)));
% fprintf('norm Juv: %d\n', norm(J_us(inner_ind_u, inner_ind_sigma)));

% fprintf('norm tau_x_df_int: %d\n', norm(tau_x_df_int(inner_ind_sigma, inner_ind_u)));
% 
% fprintf('norm Juu u_t_d2f_int: %d\n', norm(u_t_d2f_int(inner_ind_u, inner_ind_u))); 
% fprintf('norm Juu s_x_d2f_int: %d\n', norm(s_x_d2f_int(inner_ind_u, inner_ind_u))); 
% fprintf('norm Juu f_d2f_int: %d\n', norm(f_d2f_int(inner_ind_u, inner_ind_u))); 
% 
% fprintf('norm Juu df_df_int: %d\n', norm(df_df_int(inner_ind_u, inner_ind_u))); 
% fprintf('norm Juu v_t_df_int_1: %d\n', norm(v_t_df_int_1(inner_ind_u, inner_ind_u)));
% fprintf('norm Juu v_t_df_int_2: %d\n', norm(v_t_df_int_2(inner_ind_u, inner_ind_u)));


components_J_su = [];

save('comp_J_su.mat', 'tau_x_df_int');
...
Hess_J_swapped_inner = [J_uu(inner_ind_u, inner_ind_u), J_us(inner_ind_u, inner_ind_sigma); ...
                        J_su(inner_ind_sigma, inner_ind_u), J_ss(inner_ind_sigma, inner_ind_sigma)];
                    
grad_J_swapped_inner = [J_u(inner_ind_u); J_s(inner_ind_sigma)];

update_swapped_inner = - Hess_J_swapped_inner \ grad_J_swapped_inner;

% norm(update_swapped_inner)
...
% norm(Hess_J_inner)
% norm(grad_J_inner)
J_su_inner = J_su(inner_ind_sigma, inner_ind_u);
save('Hess_J_swapped_inner.mat', 'Hess_J_swapped_inner');
save('grad_J_swapped_inner.mat', 'grad_J_swapped_inner');

%%%%%%%%%%%%%%%%%%%%%%% SOLVE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
update_inner = - Hess_J_inner \ grad_J_inner;
% norm(update_inner)
update_sigma_inner = update_inner(1:length(inner_ind_sigma));
update_u_inner = update_inner(length(inner_ind_sigma)+1:end);

norm_update_u = norm(update_sigma_inner)
norm_update_sigma = norm(update_u_inner)
...
%save('my_variables.mat', 'update', 'J_ss_no_bdy', 'J_su_no_bdy', 'J_us_no_bdy', 'J_uu_no_bdy', 'Hess_J_no_bdy', 'J_s_no_bdy', 'J_u_no_bdy', 'J_eval_temp', 'sigma', 'my_u');
