...
delta_t = 10^(-2);
no_time_st = 100;
ov_time_st = no_time_st + 1;
t = linspace(0, (no_time_st+1)*delta_t, no_time_st+1);
...
left_bdy = ones(ov_time_st, 1);
right_bdy = ones(ov_time_st, 1);

f(:,1) = left_bdy;
f(:, end) = right_bdy;

f(1,1:pts) = u0;
f_vec = reshape(f', [(pts*ov_time_st),1]);
%f(ceil(N/2)) = 4;
...
for st=2:no_time_st+1
    % for IMP EULER
    rhs =  M_h*y(:,st-1) + (delta_t*M_h*f(st, :)'); 
