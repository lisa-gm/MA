%% Sequential Implementation wave equation
...
temp = ones(Nx,1);
L_h = spdiags([r*temp, 2*(1-r)*temp, r*temp], -1:1, Nx, Nx);

% zero boundary conditions PUT BACK IN
L_h(1,:) = [1, zeros(1,Nx-1)];
L_h(end,:) = [zeros(1,Nx-1), 1];
...
ylabel('time');
zlabel('u');
title('wave equation sequential sol');
