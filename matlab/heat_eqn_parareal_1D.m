...
for st_fine=2:no_time_st_fine
    ...
    %for i=1:Nt-1
    %[y_fine(:,no_time_st_fine*(i-1)+st_fine), ~, ~] = W_cycle(A_Gauss, rhs_fine(:,i), y_fine(:,no_time_st_fine*(i-1)+st_fine-1), levels, max_iter, 'GaussSeidel');
    %end
end
...
t_fine = linspace(0,T, no_time_st_fine*(Nt-1)+1);
