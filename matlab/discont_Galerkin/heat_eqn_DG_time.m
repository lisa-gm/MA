...
%% different options for incorporating the initial conditions 
%%%%%%% OPTION 1 %%%%%%%
comp_mat_1 = comp_mat;
...
u_1 = comp_mat_1 \ rhs_1;

u_2 = comp_mat_2 \ rhs_2;
...
for ts=1:Nt-2
    ...
    u_mat_2(:, ts+1) = u_2((2*ts-1)*Nx+1:2*ts*Nx);
...
for i=1:Nt
    ...
    error_2(i) = norm(u_mat_2(:,i) - u_an(:,i));
    error_3(i) = norm(u_mat_3(:,i) - u_an(:,i));

end
% 
% norm(u_2(Nx+1:2*Nx) - u_3(Nx+1:2*Nx))
% 
% norm(u_mat_2(:,2) - u_mat_3(:,2))

% figure
% plot(0:Nt-1, error_1, 0:Nt-1, error_2, 0:Nt-1, error_3);
% legend('opt 1', 'opt 2', 'opt 3');
